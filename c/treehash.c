...
int main2(int argc, char *argv[])
{
    ...
    // const char YourSecretAccessKey = "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY"; // https://docs.aws.amazon.com/amazonglacier/latest/dev/amazon-glacier-signing-requests.html#example-signature-calculation

    // HMAC(HMAC(HMAC(HMAC("AWS4" + YourSecretAccessKey,"20120525"),"us-east-1"),"glacier"),"aws4_request")

    // h1 = HMAC("AWS4" + YourSecretAccessKey,"20120525")
    // h1 = HMAC("AWS4wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY","20120525")

    // h2 = HMAC(h1,"us-east-1")

    // HMAC(HMAC(h2,"glacier"),"aws4_request")
...
int main3(int argc, char *argv[])
{
    ...
    const char fname[] = "/home/carlo/work/github/glacier-push-optimized/urandom_1000";
    ...
    // printf("file size: %lu\n", file_size);
    unsigned int nr_blocks = ((unsigned int) (ceil(((double) file_size)/ONE_MB)));
    // printf("nr blocks: %u\n", nr_blocks);
    ...
    while(1) {
        nr_read = fread(buffer, 1, ONE_MB, f);

        if (nr_read == 0) break;

        sha256(buffer, ONE_MB, &digests[offset*DIGEST_SIZE]);
    ...
    // for(offset = 0; offset < nr_blocks; offset++) {
    //     for (int i = 0; i < DIGEST_SIZE; i++)
    //         printf("%02x", digests[offset*DIGEST_SIZE + i]);
    //     printf("\n");
    // }
